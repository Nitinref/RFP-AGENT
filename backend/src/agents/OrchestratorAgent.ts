  ...
  protected async run(input: AgentInput): Promise<AgentOutput> {
    ...
    try {
      ...
      await reportService.generateReport({
        ...
        finalResponse:
          finalResponse.executiveSummary ??
    ...
    if (!parsed) {
      ...
      const response = await prisma.rFPResponse.create({
        data: {
          ...
          status: 'DRAFT',
          executiveSummary:
            "Proposal generated using fallback response due to AI formatting issue.",
          complianceStatement:
            "Compliance analysis completed with partial AI assistance.",
