...
import { PricingBreakdown } from '../types/rfp.types.js';
...
export class PricingAgent extends BaseAgent {
  ...
  protected async run(input: PricingAgentInput): Promise<AgentOutput> {
    ...
    try {
      ...
      const pricingBreakdown = await this.calculatePricing(
        skuMatches,
  ...
  private async calculatePricing(
    ...
    for (const match of skuMatches.slice(0, 3)) {
      ...
      if (product) {
        ...
        assumptions.push(
          `${product.sku}: ${quantity} units @ $${unitPrice.toFixed(2)}/unit`
        );
      }
    }

    const additionalCosts = await this.estimateAdditionalCosts(
      skuMatches,
      productsCost,
      workflowRunId
    );
  ...
  private getUnitPrice(product: any, quantity: number): number {
  ...
  private async estimateAdditionalCosts(
    skuMatches: any[],
    productsCost: number,
    workflowRunId: string
  ): Promise<{
