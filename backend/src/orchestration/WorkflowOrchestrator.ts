...
import { AgentInput } from '../types/agents.types.js';
...
export class WorkflowOrchestrator {
  ...
  private async executeWorkflow(rfpId: string, workflowRunId: string) {
    try {
  ...
  async retryWorkflow(workflowRunId: string) {
    ...
    return this.startWorkflow(
      workflowRun.rfpId,
      TriggerType.RETRY,
      `Retry of workflow run ${workflowRunId}`,
      workflowRun.createdBy || undefined
    );
  }
}
