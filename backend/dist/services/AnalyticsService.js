...
export class AnalyticsService {
     ...
     * Get RFP statistics
     */
    async getRFPStatistics(startDate, endDate) {
        const where = {};
        ...
        const [total, byStatus, byPriority, byIndustry, avgProcessingTime,] = await Promise.all([
    ...
    async getWorkflowMetrics(startDate, endDate) {
        ...
        const modelUsage = workflows.reduce((acc, w) => {
            w.modelDecisions.forEach(md => {
                if (!acc[md.chosenModel])
                    acc[md.chosenModel] = 0;
                acc[md.chosenModel]++;
            });
            return acc;
        }, {});
    ...
    async getCostAnalysis(startDate, endDate) {
        ...
        const totalCost = activities.reduce((sum, activity) => sum + parseFloat(activity.costEstimate?.toString() || '0'), 0);
        const byModel = activities.reduce((acc, activity) => {
            if (!activity.modelUsed)
                return acc;
    ...
    async getComplianceAnalysis() {
        ...
        const avgCompliance = analyses.reduce((sum, a) => sum + (a.overallCompliance || 0), 0) / analyses.length;
        const distribution = {
    ...
    async getDashboardOverview() {
        ...
        const [rfpStats, workflowMetrics, agentMetrics, costAnalysis, complianceAnalysis, upcomingDeadlines,] = await Promise.all([
