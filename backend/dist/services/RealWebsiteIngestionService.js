...
export class RealWebsiteIngestionService {
    async scanGeM() {
        ...
        const tenders = await page.$$eval(".card", (cards) => cards.slice(0, 5).map(card => ({
        ...
        for (const tender of tenders) {
            ...
            await WorkflowOrchestrator.getInstance().startWorkflow(rfp.id, TriggerType.AUTO_WEBSITE_INGESTION);
        }
        await browser.close();
    }
}
//# sourceMappingURL=RealWebsiteIngestionService.js.map